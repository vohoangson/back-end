...
public class RequestTranslationController {
	...
	@PatchMapping(UrlConstant.URL_REQUEST_TRANSLATION_YOUR_REQUEST)
	@ResponseBody
	public BaseDataResponse yourRequestTranslation(@RequestParam(defaultValue = "1", name = "page") int page,
			@RequestParam(defaultValue = "25", name = "paging") int paging,
			@CurrentUser UserPrincipal userPrincipal){
		List<RequestTranslationResponse> response = requestTranslationService.yourRequestTranslation(userPrincipal);
		return new BaseDataResponse(response);
	}
}
